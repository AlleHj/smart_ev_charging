...
)  # Använd komponent-specifik logger
...
MIN_SOLAR_CURRENT_A = (
    1  # Minsta möjliga för de flesta system är 6A, men tillåt lägre för flexibilitet.
)
...
async def async_setup_entry(
    ...
    async_add_entities(
        entities_to_add, True
    )  # True för att indikera att entiteterna ska återställas
...
class MaxPriceNumberEntity(RestoreNumber, NumberEntity):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        ...
        self._attr_unique_id = (
            f"{config_entry.entry_id}_{ENTITY_ID_SUFFIX_MAX_PRICE_NUMBER}"
        )
        ...
        self._attr_native_unit_of_measurement = (
            "SEK/kWh"  # Antag SEK, enheten kan behöva vara mer dynamisk
        )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if last_number_data is not None and last_number_data.native_value is not None:
            ...
            _LOGGER.debug(
                "Återställt värde för %s till: %s",
                self.unique_id,
                self._attr_native_value,
            )
        elif (
            self._attr_native_value is None
        ):  # Om inget återställt och inte satt i init
            self._attr_native_value = DEFAULT_MAX_PRICE
            _LOGGER.debug(
                "Inget sparat värde för %s, sätter till default: %s",
                self.unique_id,
                self._attr_native_value,
            )
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if self._attr_native_min_value <= value <= self._attr_native_max_value:
            self._attr_native_value = round(
                value,
                len(str(PRICE_STEP).split(".")[-1]) if "." in str(PRICE_STEP) else 0,
            )  # Avrunda till stegprecision
            ...
            _LOGGER.warning(
                "Ogiltigt värde för %s: %s. Tillåtet intervall: %s-%s.",
                self.name,
                value,
...
class SolarSurplusBufferNumberEntity(RestoreNumber, NumberEntity):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        self._config_entry = config_entry
        self._attr_unique_id = (
            f"{config_entry.entry_id}_{ENTITY_ID_SUFFIX_SOLAR_BUFFER_NUMBER}"
        )
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if self._attr_native_min_value <= value <= self._attr_native_max_value:
            self._attr_native_value = (
                round(value / SOLAR_BUFFER_STEP) * SOLAR_BUFFER_STEP
            )  # Säkerställ att det är en jämn multipel av steget
            self.async_write_ha_state()
            _LOGGER.info(
                "%s satt till: %s %s",
                self.name,
...
class MinSolarChargeCurrentNumberEntity(RestoreNumber, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if self._attr_native_min_value <= value <= self._attr_native_max_value:
            self._attr_native_value = (
                round(value / SOLAR_CURRENT_A_STEP) * SOLAR_CURRENT_A_STEP
            )
            self.async_write_ha_state()
            _LOGGER.info(
                "%s satt till: %s %s",
            ...
            _LOGGER.warning(
                "Ogiltigt värde för %s: %s. Tillåtet intervall: %s-%s A.",
                self.name,
                value,
                self._attr_native_min_value,
                self._attr_native_max_value,
