...
async def async_setup_entry(
    ...
    try:
        ...
        _LOGGER.debug(
            "SWITCH PLATFORM: SmartChargingEnableSwitch skapad: %s", smart_switch.name
        )

        solar_switch = EnableSolarSurplusChargingSwitch(config_entry)
        _LOGGER.debug(
            "SWITCH PLATFORM: EnableSolarSurplusChargingSwitch skapad: %s",
            solar_switch.name,
        )
        ...
        _LOGGER.debug(
            "SWITCH PLATFORM: async_add_entities har anropats för %s entiteter.",
            len(entities_to_add),
        )
    except Exception:
...
class SmartChargingBaseSwitch(SwitchEntity, RestoreEntity):
    ...
    _attr_should_poll = (
        False  # Vi förlitar oss på att HA anropar turn_on/off och återställning
    )
    ...
    def __init__(
        ...
        _LOGGER.debug(
            "%s initialiserad med unique_id %s. Initialt _attr_is_on: %s",
            self.name,
            self.unique_id,
            self._attr_is_on,
        )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        )  # Viktigt anrop till basklassen RestoreEntity

        last_state: State | None = await self.async_get_last_state()

        _LOGGER.debug(
            "RestoreState för %s (unique_id: %s): last_state är %s",
            self.name,
            self.unique_id,
        ...
        if last_state is not None and last_state.state is not None:
            if last_state.state == STATE_ON:
                ...
                _LOGGER.warning(
                    "Oväntat sparat tillstånd '%s' för %s (%s), använder default: %s",
                    last_state.state,
                    self.name,
                    self.unique_id,
                    self._attr_is_on,
            ...
            _LOGGER.debug(
                "Inget giltigt sparat tillstånd hittades för %s (%s), använder default: %s",
                self.name,
                self.unique_id,
                self._attr_is_on,
            )
